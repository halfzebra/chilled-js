const { map, flatMap, withDefault, mapError } = require('../fun');

describe('map', () => {
	it('should transform the value', () => {
		expect(map((x) => x + ', World!')([ null, 'Hello' ])).toMatchObject([ null, 'Hello, World!' ]);
	});

	it('should return the error if the input contained the error', () => {
		expect(map((x) => x + ', World!')([ new Error('Failed to provide input'), null ])).toMatchObject([
			new Error('Failed to provide input'),
			null
		]);
	});
...
describe('flatMap', () => {
	it('should transform the value', () => {
		expect(flatMap((x) => [ null, x + ', World!' ])([ null, 'Hello' ])).toMatchObject([ null, 'Hello, World!' ]);
	});

	it('should return the error if the input contained the error', () => {
		expect(flatMap((x) => [ null, 'This worked!' ])([ new Error('Sorry! Got an error.'), null ])).toMatchObject([
	...
	it('should return the error if the map fn returned an error tuple', () => {
		expect(flatMap((x) => [ new Error('Sorry! Got an error.'), null ])([ null, 'Hello' ])).toMatchObject([
			new Error('Sorry! Got an error.'),
			null
		]);
	});
...
describe('mapError', () => {
	it('should not modify the input if it did not contain the error', () => {
		expect(mapError(() => new Error('New error!'))([ null, 1 ])).toStrictEqual([ null, 1 ]);
	});

	it('should update the error if it was passed as an argument', () => {
		expect(mapError(() => new Error('New error!'))([ new Error('Boom!'), null ])).toStrictEqual([
			new Error('New error!'),
			null
		]);
	});
